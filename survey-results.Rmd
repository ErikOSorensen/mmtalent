...
ggsave(here::here("graphs","attitudes_beliefs_by_subgroups.pdf"), width = 30, height = 50, units="cm")
```
...
ggsave(here::here("graphs","attitudes_beliefs_by_subgroups_cross.pdf"), width = 30, height = 50, units="cm")
```
...
```{r}
subjectives_dt <- long_attitudes(mmtalent) %>%
  subjectives()
...
```{r}
survey_regressions_list <- survey_regressions(subjectives_dt)
subjectives_dt %>% janitor::clean_names() %>% haven::write_dta("subjectives.dta")
...
```{r echo=FALSE}
modelsummary::modelsummary(survey_regressions_list$without_treatment,
...
modelsummary::modelsummary(survey_regressions_list$without_treatment,
                           ...
                           gof_omit = "R2 Adj.|se_type",
                           coef_map = survey_names,
                           output=here::here("tables","attitudes_controlbelief_regression.tex"))
```
...
```{r echo=FALSE}
modelsummary::modelsummary(survey_regressions_list$with_treatment,
                           stars = c('*'=0.1, '**'=0.05, '***'=0.01),
                           gof_omit = "R2 Adj.|se_type",
                           coef_map = survey_names,
...
modelsummary::modelsummary(survey_regressions_list$with_treatment,
                           ...
                           coef_map = survey_names,
                           add_rows = surv_joint_tests_wrow,
                           output=here::here("tables","attitudes_controlbelief_regression_withT.tex"))
```
...
modelsummary::modelsummary(list("A.luck"=A1, "A.talent"=A2, "A.effort"=A3, "B.luck"=B1, "B.talent"=B2, "B.effort"=B3),
                           ...
                           add_rows = wrow) %>% kableExtra::add_header_above(c(" "=1, "Unfair inequality"=3, "Belief about individual control"=3))
```
