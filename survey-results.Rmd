...
```{r Fairness views and beliefs about control, echo=FALSE}
long_attitudes(mmtalent) %>% survey_bars(numobs = FALSE, ylimits = c(0,8))
ggsave(here::here("figures","EEREV-D-24-01375_Figure_2.pdf"), width = 16, height =10, units = "cm")
```
Testing differences, unfairness:

```{r}
mmtalent |> pivot_longer(cols=c("luck_fair", "talent_fair"), names_to="a", values_to="unfairness") |>
  lm(unfairness ~ a, weight=wgt, data=_) |> summary()
mmtalent |> pivot_longer(cols=c("effort_fair", "talent_fair"), names_to="b", values_to="unfairness") |>
  lm(unfairness ~ b, weight=wgt, data=_) |> summary()
```



Testing differences, control:

```{r}
mmtalent |> pivot_longer(cols=c("luck_control", "talent_control"), names_to="a", values_to="control") |>
  lm(control ~ a, weight=wgt, data=_) |> summary()
mmtalent |> pivot_longer(cols=c("effort_control", "talent_control"), names_to="b", values_to="control") |>
  lm(control ~ b, weight=wgt, data=_) |> summary()
```
...
```{r attitude_histograms}
long_attitudes(mmtalent) %>% 
  mutate(outcometype = fct_recode(
    outcometype,
    "Belief about control" = "Belief about individual control"
  )) %>%
...
Now, it is possible to run the regression 
...
survey_names <- c('Talent'='Talent (d)',
                  ...
                  'treatmentExAnteImpersonal'	= "Treatment: Ex Ante Impersonal (d)",
                  'treatmentExPostPersonal' = "Treatment: Ex Post Personal (d)",
                  '(Intercept)'='(Intercept)')
```

```{r Attitudes Control Beliefs,  echo=FALSE}
t1 <- modelsummary::modelsummary(survey_regressions_list$without_treatment,
                           stars = c('*'=0.1, '**'=0.05, '***'=0.01),
                           gof_omit = "R2 Adj.|se_type",
                           coef_map = survey_names,
                           output = "gt") |>
  tab_spanner(label = "Unfairness of Talent,Effort,Luck determining income", columns = 2:6)
t1
t1 |> as_latex() |>
...
```{r}
names(survey_regressions_list$without_treatment[[4]]$coefficients) <- make.names(names(survey_regressions_list$without_treatment[[4]]$coefficients))
car::linearHypothesis(survey_regressions_list$without_treatment[[4]], c("Talent - Effort = 0",
                                                                        "X.Belief.about.individual.control. - Effort..Belief.about.individual.control. = 0"))
...
surv_joint_tests <- c(car::linearHypothesis(survey_regressions_list$with_treatment[[1]], 
                            c("treatmentExAntePersonal = 0",
                              ...
                              "treatmentExAnteImpersonal = 0"))$`Pr(>Chisq)`[2],
...
```{r Controlbeliefs with treatment, echo=FALSE}
...
We see that 2 out of 18 treatment effects are significant at the 10% level, which is not surprising
even under the null of no true effect, and the joint p-values are all above 10%.


## Heterogeneity of talent paradox estimates

```{r The talent paradox - heterogeneity analysis, echo=FALSE}
tar_load(survey_heterogeneity)
survey_heterogeneity$graph
ggsave(here::here("figures","EEREV-D-24-01375_Figure_3.pdf"), width=16, height =10, units="cm")
```
