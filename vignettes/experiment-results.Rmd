...
```{r}
dtm <- df %>% group_by(treatment) %>% summarize(se_gini = sd(gini, wgt)/sqrt(n()),
...
R8 <- lm(nothingtw ~ treatment + age + as.factor(income_category) + as.factor(edu_category) + as.factor(gender), data = df_right, weights=wgt)
