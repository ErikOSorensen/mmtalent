...
ggsave(here::here("figures","EEREV-D-24-01375_Figure_4.pdf"), width = 16, height = 10, units = "cm")
```

Average transfers:
```{r}
mean(mmtalent$payment_low_worker-2)
```

Modal choice pooled across treatments:
```{r}
mmtalent %>%
  mutate(transfer = payment_low_worker-2) |>
  group_by(transfer) %>%
  summarise(weighted_count = sum(wgt), .groups = "drop") %>%
  mutate(prop = weighted_count / sum(weighted_count)) |>
  dplyr::select(c(transfer,prop)) |> 
  gt() |> 
  fmt_number(decimals = 3, columns = 2)
```

Choice proportions by treatment:
...
mmtalent %>%
  ...
  mutate(prop = weighted_count / sum(weighted_count)) |>
  dplyr::select(c(treatment,transfer,prop)) |> 
  pivot_wider(
    names_from = treatment,
    values_from = prop
  ) |>
  gt() |> 
  fmt_number(decimals = 3, columns = 2:5)
```
...
We look at three different outcomes. 
The first is the gini coefficient, which (as before) is between 0 and 0.6 (there is a
base income of (2,2) that cannot be lowered, so max inequality is (8,2) or (2,8)). 
The second outcome is a dummy indicator for maximum inequality.
The third outcome is no redistribution (so the allocation will remain (8,2).
...
```{r Implemented Inequality regressions, echo=FALSE}
tar_load(implemented_inequality_list)
tests <- sapply(implemented_inequality_list, function(m) {
  car::linearHypothesis(
    m, "treatmentExAntePersonal = treatmentExAnteImpersonal"
  )[2, "Pr(>F)"]
})
row_to_add <- data.frame(
  term = "p-value: Personal = Impersonal",
  t(tests),                     # transpose → becomes one row
  check.names = FALSE
)
...
```{r Heterogeneity in treatment effects, echo=FALSE}
...
ggsave(here::here("figures","EEREV-D-24-01375_Figure_5.pdf"), width = 16, height=14, units="cm")
```


The same heterogeneity figure, but for "personal" (for appendix).

```{r Heterogeneity figure personal}
